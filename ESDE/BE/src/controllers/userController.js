...
// DONE
exports.processDesignSubmission = (req, res) => {
    console.log("processDesignSubmission is called");
    ...
    fileDataManager.uploadFile(file, async function(error, result) {
        ...
        if (error) {
            ...
            try {
                let result = await fileDataManager.createFileData(publicId, imageURL, designTitle, designDescription, userId);
...
exports.processUpdateOneUser = async(req, res, next) => {
    ...
    try {
        console.log("recordId = " + recordId + " && newRoleId = " + newRoleId);
        results = await userManager.updateUser(recordId, newRoleId);
        console.log("successfully updated");
        console.log(results);
        res.status(200).send({ message: 'Completed update' });
    } catch (error) {

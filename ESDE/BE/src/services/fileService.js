...
// User Upload File (processDesignSubmission)
...
// User Upload File (processDesignSubmission)
module.exports.createFileData = (publicId, imageURL, designTitle, designDescription, userId) => {
        ...
        return new Promise((resolve, reject) => {
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    console.log("ispdf" + (isImageUrl(imageURL)));
...
// User Search Submission (processGetSubmissionData)
module.exports.getFileData = (userId, pageNumber, search) => {
        ...
        SET @total_records =(SELECT count(file_id) FROM file WHERE created_by_id= ${userId}   );SELECT @total_records total_records; `;
            ...
            designFileDataQuery = `SELECT file_id,cloudinary_url,design_title,design_description 
            FROM file  WHERE created_by_id= ？ AND design_title LIKE concat （'%',?,'%')  LIMIT ${limit} OFFSET ${offset};
...
// Admin Search Submisson by Email (processGetSubmissionsbyEmail)
...
// ADDED
module.exports.getFileId = function(fileId) {
    console.log('getFileId method is called.');
    console.log('Prepare query to fetch created_by_id record');

    fileDataQuery = `SELECT created_by_id FROM file WHERE file_id = ?`;
    ...
    return new Promise((resolve, reject) => {
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(fileDataQuery,[fileId], (err, results) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(results);
